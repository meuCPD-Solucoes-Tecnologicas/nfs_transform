...
from nNFs import enviados_mas_sem_recibo
...
nnf_desaparecida = enviados_mas_sem_recibo
    ...
    for xmlFile in xmlFiles:
        ...
        if valores["serie"] == "1":
            ...
            with open("nNFE_atual.log", "a") as fd:
                fd.write(
                    ...
                    + str(nNFE_serie_1)
                    + " nfe original:"
                    + originalXML.get_chave_de_acesso()
                    + " []"
                    + "\n"
                )

            log(
                f"[WARNING] nota original {originalXML.get_chave_de_acesso()} teve sua complementar associada à serie {nNFE_serie_1}"
            )
            ...
            log(
                f"[WARNING] nota original {originalXML.get_chave_de_acesso()} teve sua complementar associada à serie {nNFE_serie_2}"
            )
            nNFE_serie_2 += 1
        ...
        try:
            if "--envio-producao" in args_dest or "--envprod" in args_dest:
                ...
                # pdriver.consulta(chave_de_acesso)
                __import__('ipdb').set_trace()
